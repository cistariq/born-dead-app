...
class ReportController extends Controller
{
    ...
    {
        // جلب جميع الرموز
        $allCodes = DB::table('C_ICD_CODE_TB')
            ->where('DELETE_CODE', 0)
            ->select('ICD_CD AS code', 'ICD_NAME_EN AS title')
            ->orderBy('ICD_CD')
            ->get();
        ...
        foreach ($allCodes as $code) {
            if (str_contains($code->code, '.')) {
                $parentCode = explode('.', $code->code)[0];
                $subsections[$parentCode][] = $code;
            } else {
                $sections[] = $code;
            }
        }

        return view('Report.icd_Search', compact('sections', 'subsections'));
    }
    ...
    public function search($code)
    {
        ...
        SELECT ICD_CD AS code, ICD_NAME_AR AS title
        FROM C_ICD_CODE_TB
        WHERE DELETE_CODE = 0 AND ICD_CD = :code
    ", ['code' => $code]);
    ...
    public function autocomplete($term)
    {
        ...
        SELECT ICD_CD AS code, ICD_NAME_EN AS title
        FROM C_ICD_CODE_TB
        WHERE ICD_CD LIKE :term AND DELETE_CODE = 0
        ORDER BY ICD_CD
    ...
    public function Get_Daily_Dead_Result(Request $request)
    {
        ...
        $role = [
            ...
            'Death_date_frm' => 'nullable|date_format:d/m/Y|before_or_equal:' . date('d/m/Y'),
            'Death_date_to' => 'nullable|date_format:d/m/Y|before_or_equal:' . date('d/m/Y'),
    ...
    //----------------------------------------------------------------------------
    public function Distribution_Ages(Request $request)
    {
        $user_name = Auth()->user()->user_username;
        $result['user_name'] = Auth()->user()->user_full_name;
        $result['ROW'] = BORNS_INFO_TB::GET_USER_PROFILE($user_name);
        $result['old_record'] = $request->all();
        $result['fathers'] = BORNS_INFO_TB::GET_FATHERS_AGES($request->all());
        $result['father_count'] = BORNS_INFO_TB::GET_FATHERS_COUNT($request->all());

        $result['mothers'] = BORNS_INFO_TB::GET_MOTHERS_AGES($request->all());
        $result['mother_count'] = BORNS_INFO_TB::GET_MOTHERS_COUNT($request->all());

        return view('Report.Distribution_Ages', $result);
    }
    //----------------------------------------------------------------------------
    public function Distribution_sex(Request $request)
    {
        // dd($request->all());
        $user_name = Auth()->user()->user_username;
        $result['user_name'] = Auth()->user()->user_full_name;
        ...
        //dd($result);
        $result['birthData'] = BORNS_INFO_TB::DISTRIBUTION_SEX($request->all());
        //dd($result['birthData']);
        return view('Report.Distribution_sex', $result);
    }
    //----------------------------------------------------------------------------
    public function Distribution_Weight(Request $request)
    {
        ...
        $result['ROW'] = BORNS_INFO_TB::GET_USER_PROFILE($user_name);
        $result['old_record'] = $request->all();
        $result['clinic_m'] = BORNS_INFO_TB::GET_BORNS_CLINIC_M($request->all());


        return view('Report.Distribution_Weight', $result);
    }
    //----------------------------------------------------------------------------
    public function Distribution_Clinics(Request $request)
    {
        $user_name = Auth()->user()->user_username;
        $result['user_name'] = Auth()->user()->user_full_name;
        $result['ROW'] = BORNS_INFO_TB::GET_USER_PROFILE($user_name);
        $result['old_record'] = $request->all();
        $result['borns'] = BORNS_INFO_TB::GET_BORNS_CLINIC_M($request->all());
        // dd($result['borns']);

        return view('Report.Distribution_Clinics', $result);
    }
    //----------------------------------------------------------------------------
    public function Distribution_Twins(Request $request)
    {
        $user_name = Auth()->user()->user_username;
        $result['user_name'] = Auth()->user()->user_full_name;
        $result['ROW'] = BORNS_INFO_TB::GET_USER_PROFILE($user_name);
        ...
        $result['twinsData'] = BORNS_INFO_TB::GET_TWINS_NUMBER($request->all());

        return view('Report.Distribution_Twins', $result);
    }
    //----------------------------------------------------------------------------
    public function Distribution_Region(Request $request)
    {
        $user_name = Auth()->user()->user_username;
        $result['user_name'] = Auth()->user()->user_full_name;
        $result['ROW'] = BORNS_INFO_TB::GET_USER_PROFILE($user_name);
        ...
        return view('Report.Distribution_Region', $result);
    }
    //----------------------------------------------------------------------------
    public function Distribution_Years(Request $request)
    {
        $user_name = Auth()->user()->user_username;
        $result['user_name'] = Auth()->user()->user_full_name;
        $result['ROW'] = BORNS_INFO_TB::GET_USER_PROFILE($user_name);
        ...
        $result['fathers'] = BORNS_INFO_TB::GET_FATHERS_YEARS($request->all());
        //  dd($result['fathers']);
        $result['father_count'] = BORNS_INFO_TB::GET_FATHERS_COUNT($request->all());
        // dd($result['father_count']);

        $result['mothers'] = BORNS_INFO_TB::GET_MOTHERS_YEARS($request->all());
        $result['mother_count'] = BORNS_INFO_TB::GET_MOTHERS_COUNT($request->all());

        return view('Report.Distribution_Years', $result);
    }
    //----------------------------------------------------------------------------
    public function Distribution_SexOnly(Request $request)
    {
        $user_name = Auth()->user()->user_username;
        $result['user_name'] = Auth()->user()->user_full_name;
        $result['ROW'] = BORNS_INFO_TB::GET_USER_PROFILE($user_name);
        $result['old_record'] = $request->all();
        $result['born_sex_only'] = BORNS_INFO_TB::GET_BORNS_SEXONLY($request->all());

        return view('Report.Distribution_SexOnly', $result);
    }
    //----------------------------------------------------------------------------
    public function Distribution_Job(Request $request)
    {
        $user_name = Auth()->user()->user_username;
        $result['user_name'] = Auth()->user()->user_full_name;
        $result['ROW'] = BORNS_INFO_TB::GET_USER_PROFILE($user_name);
        ...
        $result['fathers'] = BORNS_INFO_TB::GET_FATHERS_JOB($request->all());
        //  dd($result['fathers']);
        $result['father_count'] = BORNS_INFO_TB::GET_FATHERS_COUNT($request->all());
        // dd($result['father_count']);

        $result['mothers'] = BORNS_INFO_TB::GET_MOTHERS_JOB($request->all());
        $result['mother_count'] = BORNS_INFO_TB::GET_MOTHERS_COUNT($request->all());

        return view('Report.Distribution_Job', $result);
    }
}
