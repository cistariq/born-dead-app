                        ...
                        <div class="flex-column current" data-kt-stepper-element="content">
                            <div class="fv-row mb-7">
                                ...
                                <div class="row mb-6">
                                    ...
                                    <div class="col-lg-4 fv-row">
                                        <input class="form-control text-center" id="P_Date_dead"
                                            onchange="check_date();" />
                                    </div>
                                    <span class="text-danger error-text P_Date_dead_error"></span>
                                ...
                                <!--end::Input group-->
                                <div class="fv-row mb-7">
                                    <label class="d-block fw-bold fs-6 mb-5 p-3 bg-success text-white">
                                        أسباب الوفاة
                                    </label>

                                    <div class="table-responsive">
                                        <table
                                            class="table table-bordered table-striped align-middle text-center fs-6 fw-bold">
                                            <thead class="table-light">
                                                <tr>
                                                    <th style="width: 20%; text-align:center;">نوع السبب</th>
                                                    <th style="width: 20%; text-align:center;">الكود</th>
                                                    <th style="width: 60%; text-align:center;">التشخيص / الوصف</th>
                                                </tr>
                                            </thead>

                                            <tbody id="deathCausesTableBody">
                                                <!-- البيانات هنا -->
                                            </tbody>
                                        </table>
                                    </div>
                                ...
                                <div class="row mb-8">
                                    ...
                                    <div class="col-lg-4">
                                        <input type="text" name="P_advertiser_Name" id="P_advertiser_Name"
                                            class="form-control form-control-lg mb-3 mb-lg-0 text-center" disabled
                                            readonly>
                                    </div>
                                </div>
                                <div class="row mb-8">
    ...
    <script>
        $(document).ready(function() {
            ...
            if ($("#P_DEAD_NUMBER").val() != 0 || $("#P_DEAD_NUMBER").val() != '') {
                ...
                //document.getElementById('P_COMMITTE_OPINION').style.visibility = 'visible'; // اظهار رأي اللجنة في حالة التعديل


            } else {
                document.getElementById('save_btn').style.visibility = 'visible';
                document.getElementById('update_btn').style.visibility = 'hidden';
                //document.getElementById('P_COMMITTE_OPINION').style.visibility = 'hidden'; // اخفاء رأي اللجنة في حالة التعديل
        ...
        function parseDMY_HM(dateTimeStr) {
            var [datePart, timePart] = dateTimeStr.split(' ');
            var [day, month, year] = datePart.split('/').map(Number);
            var [hours, minutes] = timePart ? timePart.split(':').map(Number) : [0, 0];
        ...
        function clear_ICD2() {
            $('#DEAD_ICD2_CD').val('').trigger('change');
            var P_ICD_CODE = $('#DEAD_ICD2_CD').val();

            if (P_ICD_CODE == '') {
                $('#DIAG2_NAME').val('');
                $('#DIAG2_NAME').val(null).trigger('change');
                $('#DEAD_ICD2_CD').val(null).trigger('change');
        ...
        $('#DEAD_ICD1_CD').change(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    // $('#DEAD_ICD1_CD').prop('disabled', false);
                    //  $('#DEAD_ICD1_CD').val(null).trigger('change');
        ...
        /* document.getElementById("P_Date_dead").addEventListener("input", function() {
         let birthDate = document.getElementById("P_BIRTH_DATE").value;
         let deathDate = this.value;
         let errorMessage = document.getElementById("error-message");

         if (!birthDate) {
             errorMessage.textContent = "Please enter a birth date first.";
             this.value = ""; // Reset the death date field
             return;
         }
         ...
         if (death <= birth) {
             errorMessage.textContent = "Date of Death must be after Birth Date.";
             this.value = ""; // Reset the death date field
         } else {
        ...
        function save_dead_data() {
            ...
            if (date2 < date1) {
                Swal.fire({
                    title: 'يوجد خطأ في عملية الإدخال !',
                    text: 'تاريخ الوفاة يجب ان يكون اكبر من تاريخ الميلاد',
                    icon: 'error',
                    confirmButtonText: 'Ok'
                });
                $(this).val(''); // Reset the value
            } else if (date2 > P_DATE_OF_REPORT) {
                Swal.fire({
                    title: 'يوجد خطأ في عملية الإدخال !',
                    text: 'تاريخ الوفاة يجب ان يكون أصغر أو يساوي تاريخ التبليغ من تاريخ الميلاد',
                    icon: 'error',
                    confirmButtonText: 'Ok'
                });
                $(this).val(''); // Reset the value
