    ...
    <div class="card-header border-0 pt-6">
        ...
        <div class="card-toolbar">
            ...
            <div class="d-flex justify-content-end" data-kt-role-table-toolbar="base">
                @if (IsPermissionBtn(9))
                <!--begin::Add role-->
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#add_role_btn"
                    onClick="clear_form()">
            ...
            <div class="modal fade" id="add_role_btn" tabindex="-1" aria-hidden="true">
                <!--begin::Modal dialog-->
                <div class="modal-dialog modal-dialog-centered mw-650px">
                    <!--begin::Modal content-->
                    <div class="modal-content">
                        <!--begin::Modal header-->
                        <div class="modal-header" id="kt_modal_role_btn_header">
                            <!--begin::Modal title-->
                        ...
                        <div class="modal-body scroll-y mx-5 mx-xl-15 my-7">
                            <!--begin::Form-->
                            <form id="role_btn_form" class="form">
                                <!--begin::Scroll-->
                                <div class="d-flex flex-column scroll-y me-n7 pe-7" id="kt_modal_role_btn_scroll" data-kt-scroll="true" data-kt-scroll-activate="{default: false, lg: true}" data-kt-scroll-max-height="auto" data-kt-scroll-dependencies="#kt_modal_role_btn_header" data-kt-scroll-wrappers="#kt_modal_role_btn_scroll" data-kt-scroll-offset="300px">
                                    <!--begin::Input group-->
                                    <div class="fv-row mb-7">
                                        ...
                                        <input type="text" id="btn_name" name="btn_name" class="form-control  mb-3 mb-lg-0"/>
                                        <!--end::Input-->
                                    </div>
                                    <!--end::Input group-->
                                    ...
                                    <div class="fv-row mb-7">
                                        ...
                                        <!--end::Label-->
                                        <!--begin::Input-->
                                        <select class="form-select form-control mb-3 mb-lg-0" data-dropdown-parent="#add_role_btn"
                                            data-control="select2" id="follow_to_page" name="follow_to_page" data-placeholder="اختر الشاشة">
                                            <option value=""></option>
                                            @foreach ($rolesPage as $role)
                                                <option value="{{ $role->id }}">{{ $role->name. ' - '.$role->follow_name }}</option>
                                            @endforeach
                                        </select>
                                        <!--end::Input-->
                                    </div>
    ...
    <div class="card-body py-4">
        ...
        <table class="table align-middle table-row-dashed fs-6 gy-5" id="roles_tb">
            ...
            <tbody class="text-gray-600 fw-bold">
                <!--begin::Table row-->
                @foreach ($rolesBtn as $role)
                <tr>
                    <!--begin::role=-->
                    <td>{{ $loop->index +1}}</td>
                    <td>{{$role->name}}</td>
                    <td>{{$role->FollowToPage ? $role->FollowToPage->name:''}}</td>
                    ...
                    <td class="text-end">
                        ...
                        <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4" data-kt-menu="true">
                            ...
                            <div class="menu-item px-3">
                                <a class="menu-link px-3" onclick="updateRoleBtn({{$role->id}},'{{$role->name}}','{{$role->follow_to_page}}')" >تعديل</a>
                            </div>
                            <!--end::Menu item-->
                        </div>
                        <!--end::Menu-->
...
$(document).ready(function() {
    form = document.querySelector('#role_btn_form');
...
var handleForm = function(e) {
    ...
    function insert_new_role()
    {
        ...
        var url = "{{route('roles_btn.store')}}";
        $.ajax({
            url: url,
            type:'json',
            method: 'post',
            data: {'btn_name' : btn_name,'follow_to_page':follow_to_page ,'btn_id':btn_id},
    ...
    function clear_form() {
        $('#role_btn_form')[0].reset();
        $('#btn_id').val('');
        $('#follow_to_id').val('').change();

    }
    function updateRoleBtn(id,name,follow_to_page)
    {
        $('#btn_id').val(id);
        $('#btn_name').val(name);
        $('#follow_to_page').val(follow_to_page).change();
        $('#add_role_btn').modal('show');
